...
function renderHistory() {
  if (votingData.history.length === 0) {
    historyList.innerHTML =
      '<p class="no-history">No votes yet. Start voting!</p>';
    return;
  }

  historyList.innerHTML = votingData.history
    .map((item) => {
      ...
      return `
            <div class="history-item">
                <img class="history-thumbnail" src="${
                  item.imageUrl
                }" alt="${foodNameText}" loading="lazy" />
...
function clearHistory() {
  if (
    confirm(
      "Are you sure you want to clear all voting history and statistics? This cannot be undone."
    )
  ) {
    votingData = {
      items: {},
